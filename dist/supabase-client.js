...
const _AppState = class _AppState {
  ...
  constructor() {
    ...
    this.candyData = /* @__PURE__ */ new Map();
  }
  /**
  ...
  getAuthStatus() {
    ...
    return {
      ...
      userEmail: ((_a = this.currentUser) == null ? void 0 : _a.email) || null,
   ...
   */
  getCache() {
    return _AppState.cache;
  }
};
// Caches data to reduce redundant requests.
__publicField(_AppState, "cache", /* @__PURE__ */ new Map());
...
class SupabaseQueryBuilder {
  ...
  // Filter methods
  eq(col, val) {
    return this._addFilter(col, "eq", val);
  }
  neq(col, val) {
    return this._addFilter(col, "neq", val);
  }
  gt(col, val) {
    return this._addFilter(col, "gt", val);
  }
  gte(col, val) {
    return this._addFilter(col, "gte", val);
  }
  lt(col, val) {
    return this._addFilter(col, "lt", val);
  }
  lte(col, val) {
    return this._addFilter(col, "lte", val);
  }
  like(col, pat) {
    return this._addFilter(col, "like", pat);
  }
  ilike(col, pat) {
    return this._addFilter(col, "ilike", pat);
  }
  ...
  // CRUD operations
  async insert(data) {
    return this._request("POST", data, { "Prefer": "return=representation" });
  }
  async update(data) {
  ...
  async _request(method, body, extraHeaders = {}, useFilters = false) {
    ...
    try {
      ...
      const result = ((_a = response.headers.get("content-type")) == null ? void 0 : _a.includes("application/json")) ? await response.json() : null;
      if (!response.ok) {
        if (response.status === 406 && this.query.single) {
          return { data: null, error: { code: "PGRST116", message: "No rows found" } };
        }
        throw new Error((result == null ? void 0 : result.message) || "Request failed");
      }
  ...
  _buildQueryString() {
    ...
    this.query.filters.forEach((f2) => params.append(f2.split("=")[0], f2.split("=")[1]));
  ...
  async _getHeaders() {
    var _a;
    const token = ((_a = this.auth.currentSession) == null ? void 0 : _a.access_token) || this.key;
    return {
...
class SupabaseAuth {
  ...
  async _authRequest(endpoint, payload, event) {
    try {
      ...
      if (sessionData == null ? void 0 : sessionData.access_token) {
        const session = { ...sessionData, expires_at: Math.floor(Date.now() / 1e3) + sessionData.expires_in, user: data.user };
  ...
  // Storage helpers
  _getStoredSession() {
    return new Promise((resolve) => chrome.storage.local.get("supabase_session", (r2) => resolve(r2.supabase_session)));
  }
  _storeSession(session) {
    return new Promise((resolve) => chrome.storage.local.set({ supabase_session: session }, resolve));
  }
  _removeStoredSession() {
    return new Promise((resolve) => chrome.storage.local.remove("supabase_session", resolve));
  }
  _isValidSession(session) {
    return (session == null ? void 0 : session.expires_at) && Date.now() / 1e3 < session.expires_at;
  }
  _notifyListeners(event, session) {
    setTimeout(() => this.listeners.forEach((cb2) => cb2(event, session)), 0);
  }
}
const supabase = {
