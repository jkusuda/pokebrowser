...
import { C as CANDY_FAMILY_MAP } from "./evolution-data.js";
...
async function ensureAuthReady(maxRetries = 5, retryDelay = 1e3) {
  ...
  try {
    ...
    const response = await fetch(`${CONFIG.SUPABASE_URL}/rest/v1/pokemon_history`, {
      ...
      body: JSON.stringify({
        ...
        first_caught_at: (/* @__PURE__ */ new Date()).toISOString()
      })
    });
    console.log(`📊 Response status: ${response.status} ${response.statusText}`);
    if (response.ok) {
      console.log(`✅ Added Pokemon ${pokemonId} to history`);
      const { pokemonHistory: pokemonHistory2 = [] } = await chrome.storage.local.get(["pokemonHistory"]);
      if (!pokemonHistory2.includes(pokemonId)) {
        pokemonHistory2.push(pokemonId);
        await chrome.storage.local.set({ pokemonHistory: pokemonHistory2 });
...
async function deductCandy(pokemonId, amount) {
  ...
  try {
    ...
    if (getCurrentResponse.ok) {
      ...
      if (existingRecord && existingRecord.candy_count !== void 0) {
...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
  if (request.type === "AUTH_STATE_CHANGED") {
    ...
    console.log("🔍 Background: Current auth state - User:", (currentUser == null ? void 0 : currentUser.email) || "none", "Token:", authToken ? "present" : "none");
